...
public class Acyclic {
	...
	/*
	 * Returns a set of nodes in an array where the node at the beginning of 
	 * the array has the smallest finish time and the last node has the largest 
	 * finish time.
	 * 
	 * Input:	An array where the nodes will be stored. It must have enough space
	 * 			to store all the nodes.
	 * 
	 * Output:	An array that contains nodes of a graph where the node at the be-
	 * 			ginning of the array has the smallest finish time and the last one
	 * 			the largest finish time.
	 */
	public int[] getNodeFinishedTime(int[] arr) {
		for (int i = 0;i < Black.size();i++) {
			arr[i] = Black.elementAt(i);
		}
		return arr;
	}
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			System.out.println(checkAcyclic.DFS(adj_matrix));
		}
